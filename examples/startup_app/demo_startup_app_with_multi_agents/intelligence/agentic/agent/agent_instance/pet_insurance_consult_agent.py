...
class PetInsuranceConsultAgent(Agent):
    ...
    def execute(self, input_object: InputObject, agent_input: dict, **kwargs) -> dict:
        ...
        # 1. planning agent.
        planning_agent_res = AgentManager().get_instance_obj('pet_insurance_planning_agent').run(**agent_input)
        ...
        executing_agent_res = AgentManager().get_instance_obj('pet_insurance_executing_agent').run(**agent_input)
        agent_input['search_context'] = executing_agent_res.get_data('search_context')

        # 3. expressing agent.
        expressing_agent_res = AgentManager().get_instance_obj('pet_insurance_expressing_agent').run(**agent_input)
